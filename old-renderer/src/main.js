...
function render() {
    ...
    let headingTransform = new Matrix3(Math.cos(heading), 0, -Math.sin(heading), 0, 1, 0, Math.sin(heading), 0, Math.cos(heading));
    let pitch = rotationY * (3.14159 / 180);
    let pitchTransform = new Matrix3(1, 0, 0, 0, Math.cos(pitch), Math.sin(pitch), 0, -Math.sin(pitch), Math.cos(pitch));
    let transform = headingTransform.multiply(pitchTransform);
    transform.print();
    cubes.forEach(cube => {
        cube.getFaces().forEach(plane => {
            let point1 = transform.worldToScreen(plane.v1);
            let point2 = transform.worldToScreen(plane.v2);
            let point3 = transform.worldToScreen(plane.v3);
            let point4 = transform.worldToScreen(plane.v4);
    ...
    triangles.forEach(triangle => {
        let point1 = transform.worldToScreen(triangle.v1);
        let point2 = transform.worldToScreen(triangle.v2);
        let point3 = transform.worldToScreen(triangle.v3);
        renderer.draw_line([point1.x, point1.y], [point2.x, point2.y], 1);
        renderer.draw_line([point3.x, point3.y], [point2.x, point2.y], 1);
        renderer.draw_line([point3.x, point3.y], [point1.x, point1.y], 1);
    });
